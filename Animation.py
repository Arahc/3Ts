...
class moveAnimation:
    ...
    def __init__(self, object:Img, animateSeconds:float, nowPos:tuple, endPos:tuple, name:str):
        import math
        from battleController import gSet
        self.object = object
    ...
    def update(self)->bool:
        ...
        self.framRemain -= 1
        self.nowPos = (self.nowPos[0] + self.moveXperFrame, self.nowPos[1] + self.moveYperFrame)
        self.object.setPos(self.nowPos)

        if self.framRemain == 0:
            self.object.setPos(self.endPos) # Ensure the object reaches the end position
            return True
        return False

class fadeAnimation:
    from Img import Img
    def __init__(self, object:Img, animateSeconds:float, nowAlpha:int, endAlpha:int, name:str):
        import math
        from battleController import gSet
        self.object = object
        self.framRemain = math.ceil(animateSeconds * gSet['fps'])
    ...
    def update(self)->bool:
        ...
        if self.framRemain == 0:
            self.object.setAlpha(self.endAlpha) # Ensure the object reaches the end alpha
            return True
        return False