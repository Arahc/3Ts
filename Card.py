...
def cardAttack(gate:str, card0:Card, card1:Card, card2:Card=None, time:int=0):
    ...
    resultCard = Card(ID=0, title=f'生成卡[{time}]', description=card1.title + ("" if card2 is None else "+"+card2.title), gate='None', type=True, level=0, effects=[], img=Img(pg.image.load('./assets/card/1.png').convert_alpha(), (75, 75), (0, 0)))
    if gate == 'not':
