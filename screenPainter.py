...
def drawBattleInfo(friendUnit:Character, enemyUnit:Character, enemyID:int, enemyNumber:int):
    """
    ...
    if friendUnit.HP > 0 and friendUnit.HPlossThisTurn > 0:
        ...
        pg.draw.rect(
            ...
            gSet['--HP-third'],
            (
                10 + barWidth * friendHPratio - usedWidth,
                10,
                usedWidth,
                barHeight
            )
    ...
    pg.draw.rect(gSet['screen'], gSet['--SP-front'], (10, 40, barWidth * friendSPratio, barHeight))
    ...
    if enemyUnit.HP > 0 and enemyUnit.HPlossThisTurn > 0:
        ...
        pg.draw.rect(
            ...
            gSet['--HP-third'],
            (
                gSet['screenWidth'] - barWidth - 10 + barWidth * enemyHPratio - usedWidth,
                10,
                usedWidth,
                barHeight
            )
    ...
    pg.draw.rect(gSet['screen'], gSet['--Enemy-number-ground'], (gSet['screenWidth'] - barWidth - 10, 40, barWidth, barHeight))
    pg.draw.rect(gSet['screen'], gSet['--Enemy-number-front'], (gSet['screenWidth'] - barWidth - 10, 40, barWidth * enemyNMratio, barHeight))
    ...
    friendHPtext = gSet['font-battleInfo'].render(friendHPtextContent, True, gSet['--font-color-light'])
    friendSPtextContent = f'SP: {friendUnit.SP}'
    if friendUnit.SPlossThisTurn() > 0:
        friendSPtextContent += f' (-{friendUnit.SPlossThisTurn()})'
    friendSPtext = gSet['font-battleInfo'].render(friendSPtextContent, True, gSet['--font-color-light'])
    ...
    gSet['screen'].blit(enemyHPtext, (gSet['screenWidth'] - enemyHPtext.get_width() - 10, 3))
    gSet['screen'].blit(enemyNMtext, (gSet['screenWidth'] - enemyNMtext.get_width() - 10, 33))
...
def drawCards(friendUnit: Character, enemyUnit: Character):
    ...
    for i, card in enumerate(friendUnit.onHandCards):
        ...
        maxCardsPerRow = max((gSet['screenWidth'] // 2 - 180) // (cardWidth + cardSpacing), 1)
...
def drawCardBorders(friendUnit: Character, enemyUnit: Character, selectedCards:list, hoveredCards:list):
    """
    ...
    for i, card in enumerate(friendUnit.onHandCards):
        cardWidth, cardHeight = gSet['cardSize']
        maxCardsPerRow = max((gSet['screenWidth'] // 2 - 180) // (cardWidth + cardSpacing), 1)  # 每行最多显示的卡牌数量
        row = i // maxCardsPerRow
        col = i % maxCardsPerRow
        x = 10 + col * (cardWidth + cardSpacing)
        y = gSet['screenHeight'] - cardHeight - 10 - row * (cardHeight + cardSpacing)
...
def drawCardDetail(card: Card, cardFrom: bool): # True = friend | False = enemy
    ...
    for line in cardInfo:
        infoText = gSet['font-cardDetail'].render(line, True, gSet['--font-color-light'])
